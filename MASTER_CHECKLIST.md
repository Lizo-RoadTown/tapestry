...
- [~] **Step 2** — agent-context MCP — code MERGED (PR #5/#7) + **staging deployed + parity GREEN 2026-06-20** (`tapestry-agent-context-staging`; /health, write/read/recall, MCP handshake all green; `tapestry_step2_staging_parity_verified_2026_06_20`). **Only the prod re-source remains — gated on operator authorization + the-loom blueprint handover** (loom-agent briefed: `tapestry_agent_to_loom_agent_step2_prod_cutover_handover_2026_06_20`).
- [x] **Step 3** — project-registry — MERGED to main (PR #8, `2f89426`): verbatim Lift + `002_init_projects.sql`. No deploy. Net-new signup endpoint deferred.
- [~] **Step 4** — engine — **Refactor DONE 2026-06-21** (branch `migration/04-engine`, PR open): `engine/skill-compiler/` + `services/skill-making/` lifted with import paths rewritten to Tapestry layout (`python/<pkg>/` + sys.path bootstrap); **bridge `hmac_verify`+`models` byte-identical** (wire contract); compile + import-resolution verified; drift-watcher clean. No deploy. **Open: deploy shape** (bridge is mounted-library in Make_Skills, not standalone) + bridge DB tables (`bridge_idempotency`/`promoted_skills`) forklift. Runbook: `runbooks/04-engine.md`.
