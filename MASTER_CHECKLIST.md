...
- [~] **Step 4** — engine — **Refactor DONE 2026-06-21** (PR #9): `engine/skill-compiler/` + `services/skill-making/` lifted with import paths rewritten to Tapestry layout (`python/<pkg>/` + sys.path bootstrap); **bridge `hmac_verify`+`models` byte-identical** (wire contract); compile + import-resolution verified; drift re-verified clean. No deploy. **Open: deploy shape** (bridge is mounted-library in Make_Skills, not standalone) + bridge DB tables (`bridge_idempotency`/`promoted_skills`) forklift. Runbook: `runbooks/04-engine.md`.
