...
| `services/self-observer/` (cron) | `the-loom/services/self-observer/{main,github_scanner,signal_rules,candidate_client,memory_client,synthesis,telemetry_client,config}.py` | 1,465 LOC | runtime (Render cron `0 */6 * * *`, plan starter) | live + producing candidates | maps to `tapestry/services/project-observatory/` per MANIFESTO §4.3, NOT to a dedicated self-observer dir (per `feedback_tapestry_is_canonical...` mapping: "project-observatory" carries this role) | heavy (1,465 LOC across 8 files; new B2 `memory_client.py` + `synthesis.py` from commit `478d7e3` are tightly coupled to agent-context URL) | none |
