...
| 7a | **Telemetry-ingestion + project-observatory import.** Move both into Tapestry. Per Open Question 4, may pull earlier (bundled with Step 4) so the engine's telemetry-sender targets a Tapestry endpoint directly. | Tapestry-agent | loom-source steward | Step 4 done (or earlier) | M (2-3 PRs) |
| 8 | **Discipline plugins → `integrations/claude-code/discipline/{loom,make-skills}/`.** Tapestry CI publishes to marketplace repo (which stays as distribution endpoint, not source). | Tapestry-agent | both source stewards, one flavor each | Step 2 done (so URL config resolves to Tapestry MCP) | M (2-3 PRs) |
